...
import { mockSubjectCompletions } from "@/app/data/mockData";
export const SubjectCompletions = () => {
  const [selectedSubjectForReport, setSelectedSubjectForReport] = useState(null);
  const [materialsSubjectId, setMaterialsSubjectId] = useState(null);
  ...
  />
    ...
    >
      ...
      /* Subject Footer Actions */
                    ...
                    <div className="p-4 bg-white border-t border-slate-100 flex gap-3">
                      <button
                        onClick={() => setMaterialsSubjectId(subject.id)}
                        className="flex-1 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-all flex items-center justify-center gap-2 text-sm"
                      >
    ...
    /* Achievement Badges */
      ...
      {materialsSubjectId && (
        <StudyMaterialsModal
          subjectId={materialsSubjectId}
          onClose={() => setMaterialsSubjectId(null)}
        />
      )}
    </div>;
};

/** Fetches and displays study materials for a subject */
const StudyMaterialsModal = ({ subjectId, onClose }) => {
  const { data: subjectDetails, isLoading } = useQuery({
    queryKey: ["subject-materials", subjectId],
    queryFn: async () => {
      // Try fetching with the mock ID first — if it fails, it's fine
  ...
  const materials = subjectDetails?.studyMaterials || [];

  const getTypeIcon = (type) => {
    switch (type) {
      case 'PDF': return <FileText className="w-5 h-5 text-red-600" />;
      case 'Video': return <Video className="w-5 h-5 text-blue-600" />;
      default: return <LinkIcon className="w-5 h-5 text-amber-600" />;
    }
  };
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">{subjectDetails?.name || 'Subject'}</p>
            </div>
          </div>
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto">
          {isLoading ? (
            ...
            <div className="space-y-3">
              {materials.map((mat, idx) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-3 p-4 border border-slate-200 rounded-xl hover:bg-slate-50 hover:border-blue-200 transition-all group"
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${getTypeBg(mat.type)}`}>
                    {getTypeIcon(mat.type)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-slate-800 text-sm truncate">{mat.title}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{mat.type}</p>
                  </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="font-bold text-slate-500 text-sm">No study materials available</p>
              <p className="text-xs text-slate-400 mt-1">Your teacher hasn't uploaded materials for this subject yet.</p>
            </div>
          )}
        </div>
